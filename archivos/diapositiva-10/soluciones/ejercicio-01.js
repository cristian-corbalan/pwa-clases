...
function compartirFavoritos() {
  ...
  for (const libro of libros) {
    mensaje += `- ${libro.titulo}, de ${libro.autor}. ${libro.enlace}\n`;
  }

  // Creamos el objeto de compartir
  const datosCompartir = {
    title: 'Mis libros favoritos',
    text: mensaje,
  };

  // Ejecutamos el método, enviando el objeto creado
  if (navigator.share) {
    navigator
      .share(datosCompartir)
      .then(() => {
        console.log('Libros compartidos con éxito');
      })
      .catch((error) => {
        console.warn('Hubó un error al compartir los libros', error);
      });
  }
}
